...
const BAR_GRADIENTS = [
  "from-[#5A0008] via-[#5A0008] to-[#A60F1A]", // 1. deep burgundy    — top bar
  "from-[#5A0008] via-[#7A0C14] to-[#A60F1A]", // 2. dark wine
  "from-[#5A0008] via-[#A60F1A] to-[#A60F1A]", // 3. crimson
  "from-[#5A0008] via-[#A60F1A] to-[#D65A61]", // 4. mid red
  "from-[#1A1A1A] via-[#7A0C14] to-[#D65A61]", // 5. soft rose         — bottom bars
];
...
function BarColumn({
  ...
  return (
    ...
    >
      ...
      >
        <div className="whitespace-nowrap rounded-[10px] border border-white/[0.12] bg-[#1A1A1A]/90 px-2.5 py-1.5 shadow-[0_6px_20px_rgba(0,0,0,0.60)] backdrop-blur-md">
        ...
        <div className="absolute -bottom-[5px] left-1/2 h-2 w-2 -translate-x-1/2 rotate-45 border-b border-r border-white/[0.12] bg-[#1A1A1A]" />
      </div>

      {/* Bar — grows upward from base on hover */}
      <div
        className="relative w-full origin-bottom transition-[transform,filter] duration-300 ease-out group-hover/bar:scale-y-[1.12] group-hover/bar:[filter:drop-shadow(0_6px_18px_rgba(166,15,26,0.30))]"
